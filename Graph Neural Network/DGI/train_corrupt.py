...
def corrupt_data_diff_label(feat):
    '''
    label이 동일하지 않은 group을 corrupt하여 할당하는 함수
    '''
    for num in range(n_class):
        df_feature = pd.DataFrame(features.numpy())
        df_feature['label'] = labels
        labeld_df = df_feature[df_feature['label'].apply(lambda x: x==num)] 
        not_labeld_df = df_feature[df_feature['label'].apply(lambda x: x!=num)]
        original_index = labeld_df.index
        labeld_df = not_labeld_df.sample(len(labeld_df)) #label이 아닌 것으로 대체
        labeld_df.index = original_index
        if num ==0:
            all_df = labeld_df
        else:
            all_df = pd.concat([all_df, labeld_df])
    all_df.sort_index(inplace=True)
    all_df.drop(columns='label', inplace=True)
...
def train(epoch):
    ...
    model.train()
    optimizer.zero_grad()
     # Corrupt input graph (shuffling for row groupby labels)
